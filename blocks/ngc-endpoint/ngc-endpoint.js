...
// import { getProduct } from '../productgrid/productgrid.js'
...
const productTileMarkup = (product) => {
  ...
  <a href="${productLink}">
    <img loading="lazy" src="${product.thumbnail.url}" alt="" title="${product.name}" />
  </a>
...
export async function addToShoppingCart(sku, quantityToAdd) {
  ...
  const productDet ={sku, quantity: quantityToAdd, selected_options: []};
  ...
  if(productItems.length != 0){
    productItems.forEach((item, index) => {
      if (item.product.image !== null && item.product.image.url.split("/").pop() !== "image.jpg") {
        ...
        const productTile = productTileMarkup(item.product);
      ...
      addToCartBtn[index].addEventListener('click', async () => {
        // const prod = await getProduct(item.product.sku);
        addToShoppingCart(item.product.sku);
      })
