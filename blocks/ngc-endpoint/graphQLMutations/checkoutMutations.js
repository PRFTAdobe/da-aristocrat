...
const getPriceSummary = async (cartID) => {
  ...
  const checkoutCart = await utilities.fetchRequests(utilities.GRAPHQL_ENDPOINT, 'POST', utilities.HEADERS, query);
...
const placeOrder = async (cartID) => {
  ...
  const placeOrderResponse = await utilities.fetchRequests(utilities.GRAPHQL_ENDPOINT, 'POST', utilities.HEADERS, query);
...
const getCartCheckoutDetails = async (cartID) => {
  ...
  const cartResponse = await utilities.fetchRequests(utilities.GRAPHQL_ENDPOINT, 'POST', utilities.HEADERS, query);
